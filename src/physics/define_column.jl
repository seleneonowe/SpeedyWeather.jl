...
Base.@kwdef mutable struct ColumnVariables{NF<:AbstractFloat} <: AbstractColumnVariables
    ...
    const bulk_cloud_fraction::Vector{NF} = zeros(NF, nlev) # fraction of grid cell that is cloudy [0-1]
    cloud_cover::NF = 0                     # fraction of the surface with cloud overhead [0-1]
